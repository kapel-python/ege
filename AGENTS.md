...
- test: `node test/smoke.js` и `node test/recommender.js` (сценарии движка «лучший следующий шаг»), `python3 test/auth.py` (аккаунты: guest→register→login→logout, сессии, миграция), `python3 test/onboard-subject.py` (регистрация во втором предмете), `python3 test/domain-state.py`, `python3 test/occ.py` или `python3 -m py_compile server/server.py`
