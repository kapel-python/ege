...
- test: запускать из корня репозитория; обычные команды не требуют внешних сервисов, а Python-тесты с `temp-БД` поднимают свои серверы и не трогают прод.
  ...
  - `python3 test/russian-subject.py` (API/каталог открытой практики «Итоговое сочинение», серверный приём сочинений и изоляция профиля)
  ...
  - `python3 test/ai-essay.py` (регрессия AI-оценки: transport, парсер JSON, валидатор К1–К6, детерминированная грамотность К7–К10, endpoint POST /api/ai/essay; офлайн, temp-БД; live-режим — `EGE_AI_LIVE=1`)
